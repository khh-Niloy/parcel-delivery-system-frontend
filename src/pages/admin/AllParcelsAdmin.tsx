...
export default function AllParcelsAdmin() {
  const { data, isLoading, isError, refetch } =
    useAllParcelsAdminQuery(undefined);
  console.log(data)
        ...
        <TableBody>
          {parcels.map((p) => {
            const hasTrackingEvents =
              Array.isArray(p?.trackingEvents) && p.trackingEvents.length > 0;

            const currentStatus: string = p?.status;
            const nextStatuses: string[] = Array.isArray(
              StatusFlow[currentStatus as keyof typeof StatusFlow]?.next
            )
              ? StatusFlow[currentStatus as keyof typeof StatusFlow].next
              : [];
            const allowedRolesForCurrent: string[] = Array.isArray(
              StatusFlow[currentStatus as keyof typeof StatusFlow]?.allowedRoles
            )
              ? StatusFlow[currentStatus as keyof typeof StatusFlow]
                  .allowedRoles
              : [];
            const userRole: string | undefined = meData?.data?.role;
            const canChangeThisStatus = userRole
              ? allowedRolesForCurrent.includes(userRole)
              : false;
            ...
            const handleChangeStatus = async (nextStatus: string) => {
              try {
                ...
                toast.error(
                  error?.data?.message || "Failed to update parcel status"
                );
                console.error(error);
              } finally {
                ...
                <TableRow key={(p?.trackingId ?? p?._id) + "-main"}>
                  ...
                  <TableCell>
                    <div className="space-y-1">
                      ...
                      <div className="text-xs text-muted-foreground">
                        <span>
                          Sender:{" "}
                          {p?.senderId?.name || p?.senderId?.email || "N/A"}
                        </span>
                  ...
                  <TableCell>
                    <div className="space-y-1">
                      <div>
                        <strong>From:</strong>
                        <span
                          className="text-sm text-muted-foreground block whitespace-pre-wrap break-words"
                          title={p?.pickupAddress.address}
                        >
                      ...
                      <div>
                        ...
                        <span
                          className="text-sm text-muted-foreground block whitespace-pre-wrap break-words"
                          title={p?.deliveryAddress.address}
                        >
                  ...
                  <TableCell>
                    <div className="space-y-1">
                      <div>
                        <span
                          className={`inline-flex items-center gap-1.5 px-3 py-1.5 text-xs font-semibold rounded-full ring-1 ring-inset ${
                            p?.status === "REQUESTED"
                      ...
                      <div className="text-xs text-muted-foreground">
                        <div>
                          Delivery:{" "}
                          {p?.deliveryDate
                            ? format(new Date(p.deliveryDate), "MMM dd")
                            : "-"}
                        </div>
                        <div>
                          Created:{" "}
                          {p?.createdAt
                            ? format(new Date(p.createdAt), "MMM dd")
                            : "-"}
                        </div>
                        <div>
                          Updated:{" "}
                      ...
                      <div className="space-y-4">
                        ...
                        <div className="space-y-4">
                          {p.trackingEvents.map((event: any, index: number) => {
                            const isLast =
                              index === p.trackingEvents.length - 1;
                            return (
